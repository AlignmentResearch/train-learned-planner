...
requirements.txt.new: pyproject.toml
	docker run -v "${HOME}/.cache:/home/dev/.cache" -v "$(shell pwd):/workspace" "ghcr.io/astral-sh/uv:python${PYTHON_VERSION}-bookworm" \
    sh -c "cd /workspace \
		&& uv pip compile --verbose -o requirements.txt.new pyproject.toml"
...
local-install: requirements.txt
	...
	# uv pip install https://github.com/AlignmentResearch/envpool/releases/download/v0.1.0/envpool-0.8.4-cp310-cp310-linux_x86_64.whl
