...
for env_seed, learn_seed in [(random_seed(), random_seed()) for _ in range(2)]:
    for yang_init in [True, False]:
        for normalize_input in [False]:
            network = GuezResNetConfig(yang_init=yang_init, norm=IdentityNorm(), normalize_input=normalize_input)
...
for i in range(0, len(clis), RUNS_PER_MACHINE):
    runs.append(
        FlamingoRun(
            ...
            XLA_PYTHON_CLIENT_MEM_FRACTION='".47"',
        )
    )


GROUP: str = group_from_fname(__file__, "255")
