...
class Rollout(NamedTuple):
    obs: jax.Array
    dones: jax.Array | NDArray
    actions: jax.Array
    logitss: jax.Array
    env_ids: jax.Array | NDArray
    rewards: jax.Array | NDArray
...
def impala_loss(
    ...
    discount_t = (~minibatch.dones)[1:] * args.gamma
    mask_t = jnp.ones_like(discount_t)  # Don't mask any loss timesteps -- we use them all.

    logits_to_update, value_to_update = jax.vmap(get_logits_and_value, in_axes=(None, 0))(params, minibatch.obs)
    ...
    a_t = minibatch.actions[:-1]
    rhos_tm1 = rlax.categorical_importance_sampling_ratios(logits_to_update, minibatch.logitss[:-1], a_t)
    ...
    them here.
    """
    vtrace_returns = vtrace_td_error_and_advantage(v_tm1, v_t, minibatch.rewards[1:], discount_t, rhos_tm1)
